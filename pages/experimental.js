...
// TipTap import (optional): If not using, just comment out
...
export default function ExperimentalLegacy() {
  // Legacy/TipTap switcher
  const [mode, setMode] = useState("legacy"); // legacy | tiptap
  ...
  function acceptEdit(idx) {
    ...
    setEdits(newEdits);
    setActiveIdx(null);
    setLog(l => [...l, { action: "accept", idx, suggestion: edit, time: new Date() }]);
  }
  function rejectEdit(idx) {
    ...
    setActiveIdx(null);
    setLog(l => [...l, { action: "reject", idx, time: new Date() }]);
  }
  function handleRevise(idx) {
    ...
    if (revised != null) {
      ...
      setEdits(newEdits);
      setActiveIdx(null);
      setLog(l => [...l, { action: "revise", idx, suggestion: revised, time: new Date() }]);
    }
  }
  function handleUndo() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "2rem auto", padding: 24, background: "#f9fafb", borderRadius: 12 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <b>Switch editor mode:</b>
        <button style={{
          marginLeft: 12, marginRight: 8, background: mode === "legacy" ? "#a21caf" : "#e5e7eb",
          color: mode === "legacy" ? "#fff" : "#444", fontWeight: 700, border: "none", borderRadius: 6, padding: "5px 18px"
        }} onClick={() => setMode("legacy")}>Legacy (Textarea/Highlight)</button>
        <button style={{
          marginRight: 8, background: mode === "tiptap" ? "#a21caf" : "#e5e7eb",
          color: mode === "tiptap" ? "#fff" : "#444", fontWeight: 700, border: "none", borderRadius: 6, padding: "5px 18px"
        }} onClick={() => setMode("tiptap")}>TipTap (Beta)</button>
      </div>
      {mode === "legacy" && (
        <>
      ...
      {/* TipTap (native highlight version) */}
      {mode === "tiptap" && (
        <LuluTipTap
          initialText={defaultText}
          suggestions={initialEdits}
          readOnly={false}
        />
      )}
